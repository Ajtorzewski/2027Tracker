...
# Matches the donate link whether the href is relative ("/donate/slug?...") or
# absolute, and whatever the query order, as long as refcode=directory is present.
DONATE_RE = re.compile(r'/donate/([^"?/\s]+)\?[^"\s]*refcode=directory', re.I)
NAME_TITLE_RE = re.compile(r'title="Contribute to ([^"]+)"', re.I)  # fallback name source
SITE_RE = re.compile(r'href="([^"#][^"]*)"[^>]*>\s*Go to website', re.I)  # candidate website
TAG_RE = re.compile(r'<[^>]+>')
YEAR_RE = re.compile(r'\b(20\d\d)\b')
# Constant directory chrome that can bleed into the text around an entry.
...
def parse_page(page_html):
    ...
    for m in matches:
        ...
        tag_start = body.rfind("<", prev_end, m.start())   # start of the donate <a> tag
        chunk = body[prev_end:tag_start if tag_start != -1 else m.start()]
        pre = _plain(chunk)
        ym = YEAR_RE.search(pre)
        if ym:
            juris, after, year = pre[:ym.start()], pre[ym.end():], ym.group(1)
        else:
            juris, after, year = pre, "", ""
        for ph in BOILER:
            juris = juris.replace(ph, " ")
            after = after.replace(ph, " ")
        juris = re.sub(r'\s+', ' ', juris).strip().strip('">\'')
        name = re.sub(r'\s+', ' ', after).strip().strip('">\'')
        if not name:  # fallback to the donate link's title attribute
            mt = NAME_TITLE_RE.search(body[m.start():m.start() + 400])
            if mt:
                name = html.unescape(mt.group(1)).strip()
        sm = SITE_RE.search(chunk)   # the "Go to website" link, if present
        site = html.unescape(sm.group(1)).strip() if sm else ""
        rows.append({
            "name": name, "slug": slug, "jurisdiction": juris, "year": year,
            "site": site,
            "donate_url": f"https://secure.actblue.com/donate/{slug}?refcode=directory",
        })
        gt = body.find(">", m.end())          # advance past the donate tag close
        prev_end = gt + 1 if gt != -1 else m.end()
...
CHICAGO_CITYWIDE = re.compile(r"chicago[ -](mayor|city clerk|city treasurer)\b")


def is_chicago(row):
    # Match the City of Chicago only. Its label is "Chicago-<office>" (or a
    # citywide office). "Chicago Heights-..." and "North Chicago-..." are other
    # municipalities and must not match, so we anchor on an exact "chicago"
    # city segment rather than a substring.
    j = row["jurisdiction"].strip().lower()
    if not (j == "chicago" or j.startswith("chicago-") or CHICAGO_CITYWIDE.match(j)):
        return False
...
DEBUG_FIRST_PAGE = {"html": ""}   # captures page 1 raw HTML for diagnostics
...
def _pages_html(total_pages, sleep, fixture_dir):
    ...
    if fixture_dir:
        ...
        for path in sorted(glob.glob(os.path.join(fixture_dir, "*.html"))):
            with open(path, encoding="utf-8") as f:
                txt = f.read()
                if not DEBUG_FIRST_PAGE["html"]:
                    DEBUG_FIRST_PAGE["html"] = txt
                yield txt
        return
    import requests
    for p in range(1, total_pages + 1):
        ...
        r.raise_for_status()
        if not DEBUG_FIRST_PAGE["html"]:
            DEBUG_FIRST_PAGE["html"] = r.text
        yield r.text
        time.sleep(sleep)


def write_debug(path):
    """Dump a snippet of page 1 centered on the first /donate/ link, so a
    zero-result run can be diagnosed by looking at the real HTML."""
    raw = DEBUG_FIRST_PAGE["html"]
    i = raw.find("/donate/")
    start = max(0, i - 600) if i != -1 else 0
    with open(path, "w") as f:
        f.write(f"donate_link_found={i != -1}\npage_length={len(raw)}\n")
        f.write("first_500_chars:\n" + raw[:500] + "\n\n---- around first /donate/ ----\n")
        f.write(raw[start:start + 3500])
...
def main():
    ...
    if args.json:
        ...
        # If a live scan found nothing, drop a debug snippet next to the JSON
        # so the real HTML structure can be inspected.
        if not chi and not args.fixture:
            write_debug(args.json + ".debug.txt")
            print("No Chicago rows found; wrote", args.json + ".debug.txt", file=sys.stderr)
